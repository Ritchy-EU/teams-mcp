...
import type { DirectoryObject, UserSummary } from "../types/graph.js";

function toUserSummary(obj: DirectoryObject): UserSummary {
  ...
  server.tool(
    ...
    async ({ userId }) => {
      try {
        ...
        const directReports: DirectoryObject[] = response?.value ?? [];
        ...
        const reportsList: UserSummary[] = directReports.map(
          (report: DirectoryObject) => toUserSummary(report)
        );
